...
describe("Test de RUTAS", ()=>{
    describe('GET /rickandmorty/character/:id', ()=>{
        ...
        it('Si hay un error responde con status: 500', async()=>{
            await agent.get('/rickandmorty/character/13424324').expect(500);
        })
    })
    describe("GET /rickandmorty/login", ()=>{
        it("La informacion de login es correcta", async()=>{
            const response = (await agent.get("/rickandmorty/login?email=jonnier17@gmail.com&password=123asd")).body
            expect(response.access).toEqual(true)
        })

        it("La informacion de login es correcta", async()=>{
            const response = (await agent.get("/rickandmorty/login?email=pepis@gmail.com&password=123")).body;
            expect(response.access).toEqual(false)
        })
    })

    describe("POST /rickandmorty/fav", ()=>{
        const char1 = {id: "1", name: "FF"}
        const char2 = {id: "2", name: "hhh"}
    ...
    describe("DELETE /rickandmorty/fav/:id", ()=>{
        ...
        it("Elimina correctamente al personaje que se espesifica por id", async ()=>{
